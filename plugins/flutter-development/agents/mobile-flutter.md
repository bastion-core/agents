...
> BLoCs call repositories directly, helpers for complex BLoC logic. Feature-first
> organization with `common/` for cross-cutting concerns.
...
>
> BLoC + Freezed for events/states. `event.map()` for exhaustive handling, `state.when()` for
> UI consumption. Helpers for complex BLoCs (>6 handlers). Both Freezed and Equatable styles supported.

## BLoC UI Consumption

> **Full documentation**: See `context/flutter-app/state_management.md`
>
> BlocBuilder for rebuilds, BlocConsumer for side effects + rebuilds, BlocListener for pure side effects.
...
> **Full documentation**: See `context/flutter-app/state_management.md`
>
> get_it with manual service locator. `registerFactory` for BLoCs, `registerLazySingleton` for
> services/repositories. Feature-grouped registration. No injectable or auto-DI packages.
...
>
> Discovery Rule: always discover the project's UI abstraction layer before writing UI code.
> Use project-prefixed components ({Project}Button, {Project}Text, etc.), centralized colors,
> and the project's icon pack. Never use raw library components directly in feature code.
...
> **Full documentation**: See `context/flutter-app/architecture.md`
>
> 14 forbidden patterns including: separate Entity layer, Mapper classes, UseCase/Interactor classes,
> injectable/auto-DI, auto_route, ScreenUtil, data/ layer naming, core/ folder, throwing exceptions.
...
> **Full documentation**: See `context/flutter-app/state_management.md`
>
> bloc_test + mockito. 70% BLoC tests, 20% infra, 10% domain. AAA pattern, provideDummy for Result types,
> Fake for analytics/logger, Mock for repositories. Test initial state, success/failure for each event.
