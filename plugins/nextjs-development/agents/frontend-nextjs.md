...
- **Tests in proportion.** Run the tests you touched first, and the full suite once at the end; show only its summary (`| tail`). Do not re-run a suite when nothing changed since the last green run.
- **One task per invocation when possible.** If you are given several tasks, finish and commit each one before starting the next, and keep only a short summary of the finished one in mind.
...
| Zustand Stores & State Management | `context/nextjs-app/state_management.md` | When implementing stores or state patterns |
| Component, Hook & Page Patterns | `context/nextjs-app/widget_patterns.md` | When writing UI components or screens |
...
> Domain: pure types, DTOs, enums, states. Infrastructure: data-access, stores, services, helpers, UI.
> Data Flow: Component → Store → DataAccess → API. No application layer, no UseCases.

## Folder Structure

> **Full documentation**: See `context/nextjs-app/architecture.md`
>
> Module structure: `src/core/{module}/domain/` + `infrastructure/`. Routes: `src/app/[locale]/`.
> Shared code in `src/core/common/`. Path aliases: `@core/*`, `@components/*`, `~/*`.
...
>
> States modeled as discriminated unions with `kind` field. Constants for kind values (LOADING_KIND,
> LOADED_KIND, ERROR_KIND). Narrow with `state.kind === LOADED_KIND`, never instanceof.
...
> **Full documentation**: See `context/nextjs-app/state_management.md`
>
> Small stores per concern (~60-100 lines). DataAccess instantiated at module level.
> `response.fold()` directly in actions. Boolean loading flags. State+Actions in separate
...
> **Full documentation**: See `context/nextjs-app/state_management.md`
>
> Zustand for shared/persisted state, useState for temporary UI state, searchParams for
> shareable URL state (filters, pagination).
...
> **Full documentation**: See `context/nextjs-app/widget_patterns.md`
>
> 6-section structure: local state, store integration, computed values, side effects,
> handlers, return object. `use` prefix, placed in `infrastructure/ui/hooks/`.
...
> Screens orchestrate initial load with useEffect + useRef initialization guard.
...
- Colors ALWAYS from the semantic tokens below (NEVER a palette class)
...
`tailwind.config.ts`. You pick the meaning, never the color.
...
**NEVER** write `bg-white`, `bg-black`, `bg-gray-*`, `text-neutral-*`, `text-green-*`,
`border-gray-*`, a hex literal, or a hand written `dark:` variant. Each token already carries both
palettes, so a `dark:` compensation is the defect this standard removes.
...
Buttons and status pills are resolved in the shared variant, never in the screen: `<Button>` for
the primary action, `variant="approve"` and `variant="reject"` for the verdict of a review,
`variant="destructive"` for something that truly deletes.
